...
import { buildSellerListHref, type SellerListQuery } from "@/lib/admin/seller-list-params";

export function SellerListPagination({
  basePath,
  query,
  page,
  totalPages,
  ...
  return (
    <div className="flex items-center justify-between gap-3 pt-1">
      ...
      <div className="flex items-center gap-2">
        {hasPrev ? (
          <Link href={buildSellerListHref(basePath, { ...query, page: page - 1 })} scroll={false} className={`${linkClass} hover:bg-surface-elevated`}>Anterior</Link>
        ) : (
        ...
        {hasNext ? (
          <Link href={buildSellerListHref(basePath, { ...query, page: page + 1 })} scroll={false} className={`${linkClass} hover:bg-surface-elevated`}>Siguiente</Link>
        ) : (
