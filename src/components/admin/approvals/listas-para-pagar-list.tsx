...
import type { ReadyToPayItem } from "@/lib/admin/approvals";
import { MarkPaidButton } from "@/components/admin/approvals/mark-paid-button";

function timeReady(iso: string | null): string {
  if (!iso) return "—";
...
export function ListasParaPagarList({ items }: { items: ReadyToPayItem[] }) {
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-dashed px-6 py-14 text-center border-border">
        <p className="text-sm font-medium text-text-secondary">No hay ventas listas para pagar.</p>
      </div>
    );
  }
  return (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-[11px] uppercase tracking-wide text-muted-foreground border-border bg-surface">
              ...
              <th className="px-3 py-2.5 font-medium">Tiempo lista</th>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.saleId} className="border-b last:border-0 border-border/70">
                ...
                <td className="px-3 py-2.5 align-top text-muted-foreground">{timeReady(item.soldAt)}</td>
                <td className="px-3 py-2.5 text-right align-top">
                  <div className="flex flex-col items-end gap-1.5">
                    <MarkPaidButton saleId={item.saleId} saleNumber={item.saleNumber} />
                    <Link
